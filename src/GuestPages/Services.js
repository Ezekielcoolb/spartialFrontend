...
import { fetchServicepage } from "../Redux/slice/homeSlice";
...
const Services = () => {
  ...
  const { homeData, homeObject, serviceObject, loading, error } = useSelector(
    (state) => state.content || []
  );
    ...
    <ServiceRap>
      <div
        style={{
          ...
          backgroundImage: `linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.6)), url(${URL}${heading?.servicePageBanner})`,
        }}
          ...
          {services?.map((items, index) => {
            ...
            return (
              <div
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.backgroundImage = `linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.6)), url(${URL}${items?.banner})`;
